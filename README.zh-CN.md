...
Dashboard · 动态路由 · WAF 防火墙 · 2FA 验证 · 通知告警 · IP 隔离 · 自动注册
...
| **Aneiang.Yarp.Dashboard** | Web 管理面板：全量 CRUD、WAF 防火墙（`WafMiddleware`）、2FA 两步验证（`TotpHelper`）、通知告警（钉钉/通用 Webhook）、健康检查监控、熔断器管理（`CircuitBreakerMiddleware`）、代理日志（`YarpRequestCaptureMiddleware`）、审计日志、配置快照与回滚、策略引擎（`GatewayPolicyService`）、限流/重试中间件、SignalR 实时流量推送、SQLite 持久化（8 个 `Sqlite*Repository`） | 通过核心库 |
...
| **Aneiang.Yarp.Storage.Abstractions** | 存储抽象层：8 个独立仓储接口（`IRouteRepository`、`IClusterRepository` 等）+ 实体模型 | ❌ |
| **Aneiang.Yarp.Storage.Sqlite** | SQLite 存储实现：支持 SQLCipher AES-256 加密，懒加载建表，`SqliteConnectionFactory` 共享连接池 | 通过抽象层 |
...
- **TOTP 验证器**：`TotpHelper` 生成 Base32 密钥 + `otpauth://` URI，兼容 Google Authenticator、Microsoft Authenticator 等验证器应用
