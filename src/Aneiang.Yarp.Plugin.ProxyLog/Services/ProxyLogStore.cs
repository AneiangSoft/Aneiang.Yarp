...
public sealed class ProxyLogStore : IProxyLogStore
{
    ...
    /// <summary>
    /// Returns an approximate memory footprint in bytes for the buffered log entries
    /// and persistence channel backlog.
    /// </summary>
    public long GetEstimatedMemoryBytes()
    {
        var bufferedEntries = Math.Min(Volatile.Read(ref _head), _bufferLength);
        var channelBacklog = _persistenceChannel.Reader.Count;
        return (bufferedEntries + channelBacklog) * 1024L;
    }
}
