...
(function() {
    ...
    window.loadInfo = async function() {
        try {
            var result = await window.dashboardApiMethods.getInfo();
    ...
    window.loadClusters = async function() {
        try {
            ...
            var result = await window.dashboardApiMethods.getClusters();
            if (result.code !== 200) return;
            
            state.clusters.data = result.data;
            window.renderClusters();
        } catch (e) { 
            console.error('Failed to load clusters:', e);
            window.dashboardRenderers.ui.toast('Failed to load clusters', 'error');
        } finally {
            window.dashboardState.clusters.loading = false;
        }
    };
    ...
    window.renderClusters = function() {
        ...
        var clusters = state.getFilteredClusters();
        
        // Render filter toolbar
        var filterContainer = document.getElementById('cluster-filter-container');
        if (filterContainer && !filterContainer.innerHTML.trim()) {
            filterContainer.innerHTML = window.dashboardFilters.renderClusterToolbar();
            window.dashboardFilters.initClusterToolbar();
        }
        ...
        clusters.forEach(function(c) {
            ...
            var isExpanded = state.isClusterExpanded(c.clusterId);
            ...
            c.destinations.forEach(function(dest, i) {
                ...
                if (i === 0) {
                    ...
                    html += '<td rowspan="' + rowspan + '" style="font-weight:600;vertical-align:middle;">';
                    html += '<span class="cluster-expand-icon" style="display:inline-block;width:16px;">' + expandIcon + '</span> ';
                    html += window.escapeHtml(c.clusterId);
                    html += '</td>';
                }
                html += '<td><code>' + window.escapeHtml(dest.name) + '</code></td>';
                html += '<td><a href="' + window.escapeHtml(dest.address) + '" target="_blank" class="text-decoration-none">' + window.escapeHtml(dest.address) + '</a></td>';
                if (dest.health) {
                    html += '<td><span style="color:#64748b;">' + window.escapeHtml(dest.health) + '</span></td>';
                } else {
                    html += '<td>' + window.healthDot(dest.activeHealth || 'Unknown') + '</td>';
                }
                html += '<td>' + window.healthDot(dest.passiveHealth || 'Unknown') + '</td>';
                if (i === 0) {
                    html += '<td rowspan="' + rowspan + '" style="vertical-align:middle;">';
                    html += renderers.badge.health(c.loadBalancingPolicy);
                    html += '</td>';
                    html += '<td rowspan="' + rowspan + '" style="vertical-align:middle;" onclick="event.stopPropagation;">';
                    html += renderers.ui.actionButtons(
                        c.clusterId,
                        c.isEditable,
                        'editCluster(\'' + window.escapeHtml(c.clusterId) + '\')',
                        'deleteCluster(\'' + window.escapeHtml(c.clusterId) + '\')'
                    );
                    html += '</td>';
                }
                html += '</tr>';
            ...
            html += '<tr class="cluster-detail" data-cluster="' + window.escapeHtml(c.clusterId) + '" style="display:' + (isExpanded ? '' : 'none') + ';">';
            ...
            html += '<span><strong>LoadBalancingPolicy:</strong> ' + renderers.badge.health(c.loadBalancingPolicy) + '</span>';
            html += '<span><strong>Status:</strong> ' + renderers.badge.editable(c.isEditable) + '</span>';
            html += '</div>';
        ...
        document.querySelectorAll('.cluster-row').forEach(function(row) {
            row.addEventListener('click', function() {
                ...
                window.dashboardState.toggleCluster(cid);
                window.renderClusters();
            });
        });
    };
